...
当前 MVP 已完成 POSIX 空白、单双引号、反斜杠和续行分词；Windows CRT 路径反斜杠、引号及奇偶反斜杠规则；两种方言的参数引用与 argv 可逆序列化；字符级源码范围、结构化错误、资源限制和 `CommandLine` 构建器。仓库已有 15 个以上可核验的有效提交，包含可运行示例、48 项测试、native/wasm-gc 双后端检查；Windows/Linux CI 已通过。明确不实现 shell 执行、变量/通配符展开、重定向、管道或 CLI flag 语义。
