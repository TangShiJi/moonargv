...
扫描范围：接口返回的全部 2,450 个公开模块，检索模块名称、简介和关键词；另复核 2026-09-10 之后新增的 146 个模块。
...
截至查重时间，Mooncakes 没有与 MoonArgv 同名的包，也没有同时提供 POSIX/Windows 分词、可逆引用、响应文件、源码位置和输入限制的项目。相邻第三方项目及标准库 `argparse` 已在申报书中披露。本结论仅对应 2026-09-14 的公开模块状态，正式发布前仍会再次全量复核。
