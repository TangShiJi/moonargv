...
`windows_crt_vectors_test.mbt` 将 Microsoft 页面列出的 5 组命令文本/argv 示例作为外部行为对照，并在 Windows、Ubuntu CI 的两种 MoonBit 后端执行。除此之外，确定性模糊测试持续验证 `parse(join(argv)) == argv`，避免只覆盖人工挑选的样例。
